...
- A friendly modern style and, as the core presentation deliverable of the project, a faithful seventeenth/eighteenth-century period style in the tradition of Briggs (1617/1624), Vlacq (1628), Sherwin (1706), and Vega (1794): Latin column heads (Numerus / Logarithmus / Differentia), characteristic-and-mantissa notation with vinculum (overline) for negative characteristics, rounding marks on final digits raised by rounding (a deliberate project convention adopted from Babbage, 1827), difference columns in units of the last place, and column-major folio page layout.
...
The internal algorithm must retain enough guard precision to make the final rounding decision exact for the implemented integer approximation. Its public output does not expose calculation metadata, with one exception: the direction of the final rounding step (`rounded_up`) is part of each public row, because the period presentation prints a rounding mark beneath a final digit that was raised by rounding.

The displayed logarithm format supports two notation conventions. Modern decimal notation serves the modern theme. Period notation is core scope, and its conventions are selected as follows: the characteristic is printed once per value, separated from the mantissa by a comma; a comma follows every fifth mantissa digit, as Briggs and Vlacq printed (e.g. `0,30102,99957`); a negative characteristic (which arises once complement/cologarithm columns are added) is printed with U+0305 (combining overline, the vinculum) on each of its digits; a final mantissa digit that was raised by rounding carries U+0323 (combining dot below) as its rounding mark. The rounding mark is a deliberate project convention, not a period one: no seventeenth/eighteenth-century page-level exemplar of a rounding mark has been identified, so the mark is adopted from Babbage's 1827 tables (dot below a raised final digit), which postdate the cited period, and all documentation must describe it as such.
...
Converts ordered rows to HTML. Styles are selected by a named presentation theme. The modern theme is readable and responsive. The period rendering is a faithful structural reproduction of seventeenth/eighteenth-century printed tables — folio pages of column-major Numerus / Logarithmus / Differentia column groups, period notation with vinculum and comma-grouped mantissas, rounding marks (a deliberate project convention per the mathematical contract, not a period feature), serif typography, ruled borders, and restrained ink-on-paper colors — without claiming to reproduce a specific typeface or page ornament. The full structural layout is owned by the period-format phase (`08-period-format`); the `NAPIER` theme of the HTML-output phase provides period typography for the plain N/log listing only.
...
7. `07-historical-format`: the 1950s proportional school-table layout in the browser.
8. `08-period-format`: the core seventeenth/eighteenth-century period presentation — notation, folio layout, and browser route. Depends only on phases 2–3 and the existing web app, and may be implemented before phases 4–7.
...
- Whether logarithm complements (cologarithms) and interpolation tables are needed; the period notation they require (vinculum characteristics, rounding marks) is already core scope and no longer deferred.
